...
class LMTest(object):
    ...
    def __init__(self):
        ...
        # Initializing server
        self.recognition_server = rospy.Service('change_state', SetRecState, self.set_rec_state)
        # Params
    ...
    def set_rec_state(self, request):
        if self.enabled_ == True and request.state == request.STOPPED:
            rospy.loginfo("Change mode service : recognition stopped")
            self.enabled_ = False;
        elif self.enabled_ == False and request.state == request.START:
            rospy.loginfo("Change mode service : recognition (re)started")
            self.enabled_ = True;
        return 0
